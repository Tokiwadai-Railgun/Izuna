...
module.exports = {
  ...
  runInteraction: async (Izuna, interaction, guildSettings) => {
    ...
    interaction.channel
      ...
      .then((msg) => {
        if (!msg.first().mentions || !msg.first().mentions.channels)
          return interaction.message.edit({
            content: "Commande annulée, aucun salon mentionné",
            embeds: [],
          });
        Izuna.updateGuild(interaction.guild, {
          messageLogChannel: msg.first().mentions.channels.first().id,
        });
        ...
        const fourthEmbed = new EmbedBuilder()
          ...
          .addFields([
            {
              name: "Nouveau salon :",
              value: `${msg.first().mentions.channels.first()}`,
            },
          ])
          .setColor("#7F0856");
