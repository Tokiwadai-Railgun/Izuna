...
module.exports = async (Izuna) => {
    (await pglob(`${process.cwd()}/commands/*/*.js`)).map(async commandFile => {
        ...
        if (!command.description && command.type != "USER") return Logger.warn(`Command description is not defined, command not loaded. ↓\nFile: ${commandFile} \n----------`);
        ...
        if (!command.aliases && command.type != "USER") return Logger.warn(`Command aliases is not defined, command not loaded. ↓\nFile: ${commandFile} \n----------`);
